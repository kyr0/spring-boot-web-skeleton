...
public final class Whirlpool extends BaseHash {
     ...
     */
    private static final char sbox[] = new char[]
            {
                    '\u1823', '\uc6E8', '\u87B8', '\u014F', '\u36A6', '\ud2F5', '\u796F', '\u9152',
                    '\u60Bc', '\u9B8E', '\uA30c', '\u7B35', '\u1dE0', '\ud7c2', '\u2E4B', '\uFE57',
                    '\u1577', '\u37E5', '\u9FF0', '\u4AdA', '\u58c9', '\u290A', '\uB1A0', '\u6B85',
    ...
    static {
        ...
        for (int r = 1; r <= R; r++) {
            int i = 8 * (r - 1);
            rc[r] = (C[0][i + 0] & 0xff00000000000000L)
    ...
    @Override
    public byte[] hash(byte[] input) throws InvalidParameterException {
        if (null == input || input.length == 0) {
            throw new InvalidParameterException("Attempted to hash empty input");
        }
        byte[] digest = new byte[Whirlpool.DIGESTBITS >> 3];
        this.engineUpdate(input, 8 * input.length, digest);
        this.reset();
        return digest;
    }

    @Override
    public byte[] hash(String input) throws InvalidParameterException {
     ...
     */
    private void engineUpdate(byte[] source, long sourceBits, byte[] digest) {
        /*
         sourcePos
         |
         +-------+-------+-------
         ...
         */
        int bufferPos = 0;
        int bufferBits = 0; // Current number of bits on the buffer.
        ...
        final byte[] bitLength = new byte[32]; // number of hashed bits (256-bit counter).
    ...
    private void processBuffer() {
        ...
        for (int i = 0; i < 8; i++) {
            block[i] =
                    (((long) buffer[++j]) << 56L)
                            ^ (((long) buffer[++j] & 0xffL) << 48L)
                            ^ (((long) buffer[++j] & 0xffL) << 40L)
                            ^ (((long) buffer[++j] & 0xffL) << 32L)
                            ^ (((long) buffer[++j] & 0xffL) << 24L)
                            ^ (((long) buffer[++j] & 0xffL) << 16L)
                            ^ (((long) buffer[++j] & 0xffL) << 8L)
                            ^ (((long) buffer[++j] & 0xffL));
